...
from app.classification.rules import classify_page_result
from app.classification.types import PageClassificationResult, PageSignals
...
class PdfClassificationService:
    ...
    def classify_bytes(self, data: bytes) -> list[PageClassificationResult]:
        ...
        pdfplumber_features = extract_pdfplumber_page_features(data)
        page_numbers = set(pymupdf_features) | set(pdfplumber_features)
        if not page_numbers:
            page_count = 0
        else:
