...
async def test_enqueue_and_claim_returns_running_job() -> None:
    ...
    job = await queue.enqueue(pdf_id="pdf-1", job_type="process_pdf")
...
@pytest.mark.asyncio
async def test_enqueue_duplicate_pdf_id_is_idempotent() -> None:
    queue = InMemoryJobQueue()
...
async def test_fail_or_retry_requeues_until_max_attempts() -> None:
    ...
    retried = await queue.get_by_pdf_id("pdf-1")
    ...
    failed = await queue.get_by_pdf_id("pdf-1")
